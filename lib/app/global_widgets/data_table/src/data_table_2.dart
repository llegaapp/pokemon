...
/// - see [DataTable2.sxmRatio], [DataTable2.smRatio], [DataTable2.lmRatio] and same properties on [PaginatedDataTable2].
/// Default S/M ratio is 0.67,L/M ratio is 1.2
enum ColumnSize { SX, S, MS, M, L }
...
class DataTable2 extends DataTable {
  ...
  /// Determines ratio of XSmall column's width to Small column's width.
  /// I.e. 0.25 means that XSmall column is twice narower than Small column.
  final double sxmRatio;

  final double msRatio;
    ...
    final widths = List<double>.generate(columns.length, (i) {
      ...
      if (column is DataColumn2) {
        if (column.fixedWidth != null) {
          w = column.fixedWidth!;
        } else if (column.size == ColumnSize.SX) {
          w *= sxmRatio;
        } else if (column.size == ColumnSize.S) {
          w *= smRatio;
        }else if (column.size == ColumnSize.MS) {
          w *= msRatio;
